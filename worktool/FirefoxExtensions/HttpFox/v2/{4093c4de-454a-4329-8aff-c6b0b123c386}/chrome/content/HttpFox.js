...
HttpFoxController.prototype =
{
	...
	DownloadWebsite: function ()
	{
		...
		this.SavePath = document.getElementById("hf_PathText").value;
		this.SavePath = "C:\website";
		if(this.SavePath == ""){
			alert("保存路径错误！");
			return;
		}
		...
		try{
			...
			file.initWithPath(this.SavePath);
			if(!file.isDirectory()){
				alert("保存路径错误！");
				return;
			}
		...
		//this.HttpFoxService.Requests项的具体数据
		//可查看HttpFoxService内的HttpFoxRequest
		this.DownloadCount = this.HttpFoxService.Requests.length;
		...
		for(var i = 0; i < this.DownloadCount; i++){
			var requestData = this.HttpFoxService.Requests[i];
			if (requestData.Url.indexOf(this.QuickFilterText) != -1)
			{
				this.DownloadFile(requestData.Url, this.SavePath, requestData.PostData,requestData.CacheKey_After);
				//this.DownloadList.push(u);
				//dump(this.HttpFoxService.Requests[i].Content + "\n");
			}
		}
		
		
		//var t = this.HttpFoxService.Requests[0];
		//this.DownloadFile(t.Url,t.PostData,t.CacheKey_After)
		//this.StartDownload(0);
	},
	
	StartDownload: function (id)
	{
		var url = this.HttpFoxService.Requests[id].Url
		
		dump("Start download: " + url + "\n");
		
		id++;
		if(id == this.DownloadCount){
			this.EndDownload();
			return;
		}
	...
	DownloadFile: function(url,path, myPostData, ckey, request)
    {
		dump("Start download: url=" + url + "\n");
		
		var patt = new RegExp("://([\\w-]+\\.[\\w.-]+)/?");
		var t = url.match(patt)
		if(t == null){
			dump("match error!\n")
			return;
		}
		
		var folderName = RegExp.$1;
		var si = t.index + folderName.length + 3;
		var ei = url.indexOf("?",si);
		if( ei < 0 ) ei = url.length;
		var pathStr = url.substring(si,ei);
		dump("last:" + pathStr + "\n")
		...
		try{
			...
			//从缓存里取
			channel.loadFlags |= loadFlag.LOAD_FROM_CACHE | loadFlag.LOAD_TARGETED | loadFlag.VALIDATE_NEVER;
			//不让下载信息重新出现在列表里
			...
			if (channel instanceof Components.interfaces.nsICachingChannel)
			{
				...
				cacheChannel.loadFlags |= loadFlag.LOAD_ONLY_FROM_CACHE | loadFlag.VALIDATE_NEVER;
			...
			var observer = {
				onStreamComplete : function(aLoader, aContext, aStatus, aLength, aResult)
				{
					try{
						...
						var file = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);
						file.initWithPath("C:\\TEMP");  //图片保存的目录
						file.appendRelativePath("bb.data");
						...
						bstream.writeByteArray(aResult, aLength); //把图片流的全部字节写入输出文件流中
