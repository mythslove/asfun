...
HttpFoxController.prototype =
{
	...
	DownloadWebsite: function ()
	{
		...
		for(var i = 0; i < this.DownloadCount; i++){
			var u = this.HttpFoxService.Requests[i].Url;
			if (u.indexOf(this.QuickFilterText) != -1)
			{
	...
	SaveURL: function()
	{
		...
		var urlStr = "";
		var len = this.DownloadList.length
		for(var i = 0; i < len; i++){
			urlStr += this.DownloadList[i] + "\r\n";
		}
		...
		try{
			...
			stream.init(file, 0x02 | 0x08 | 0x20, 0666, 0,0);
			stream.write(urlStr,urlStr.length);
	...
	DownloadFile: function(url, myPostData, ckey, request)
    {
		dump("Start download: url=" + url + ", post data=" + myPostData + ", ckey=" + ckey + ", request="+ request + "\n");
