package 
{
	...
	public class ABCToActionScript
	{
											  ...
											  definedLocals:Object = null):Object
		{
			...
			*/
			...
			for(var iter:uint = start; iter < instructions.length; iter++)
			{
				...
				//trace2('for: '+key+' ('+stack.values.length+')');
				
				if(hitmap[iter])
				{
					//trace2('hitmap hit: '+iter);
				...
				*/
				...
				{
					//trace2('hitmapWithStack hit: '+key);
					//lines.push('HITMAPSTACK HIT');
					break;
				}
				if(iter == target)
				{
					//trace2('target hit: '+iter);
					breakOn = iter;
					break;
				}
				...
				if(showActionScript)
				{
					function branch(target1:int, target2:int, knownConditions1:Object, knownConditions2:Object):Object
					{
						...
						trace2('			branch point: '+iter);
						trace2('			start branch from: '+target1);
						...
						trace2('			end branch from: '+target1);
						trace2('			start branch from: '+target2);
						...
						trace2('			end branch from: '+target2);
					...
					function conditional(condition:String, inequality:Boolean):void
					{
						...
						if(dup)
						{
							if(condition && knownConditions.hasOwnProperty(condition))
							{
								...
								trace2('KNOWN CONDITION: '+condition+' = '+knownConditions[condition]+', skipping to: '+iter);
								return;
							}
						}
						...
						{
							trace2('CACHE MISS! EXECUTING EXPENSIVE BRANCH');
							trace2('	key: '+key2);
					...
					*/
					...
					{
						var values:Vector.<Object> = scope.values;
						var index:int =  values.length - 1 - Instruction_getscopeobject(op).index;
						if(index<0)index = 0;
						if(values.length > 0){
							stack.push(scope.values[index]);
						}
					}
		...
		//全部的类表
		public var allClsMap:Dictionary;
...
'}';
		...
		public function hasClass(name:String):Boolean{
			return this.allClsMap[name] != null;
		}
		...
		public function multinameTypeToString(r:ReadableMultiname, seperator:String = '.'):String
		{
			...
			if(this.hasClass(result)){
				...
				if(this.useClassMap[result] == null){
					this.useClassMap[result] = result;
				}
			}
...
'}';
		...
		private function trace2(...parent:Array):void{
			
			//trace.apply(null,parent);
		}
	}
}